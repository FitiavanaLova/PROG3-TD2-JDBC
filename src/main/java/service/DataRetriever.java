...
public class DataRetriever {
            ...
            if (teamRs.next()) {
                ...
                try (PreparedStatement playerStmt = conn.prepareStatement(playerSql)) {
                    ...
                    while (playerRs.next()) {
                        ...
                        player.setGoalNb(playerRs.getObject("goal_nb", Integer.class)); // ✅ AJOUT
                        player.setTeam(team);
                        players.add(player);
                    }
    ...
    public List<Player> findPlayers(int page, int size) {
        ...
        try (Connection conn = DBConnection.getDBConnection();
            ...
            while (rs.next()) {
                ...
                player.setGoalNb(rs.getObject("goal_nb", Integer.class)); // ✅ AJOUT
                players.add(player);
            }
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                for (Player player : newPlayers) {
                    ...
                    if (player.getGoalNb() != null) {
                        stmt.setInt(5, player.getGoalNb());
                    } else {
    ...
    public List<Player> findPlayersByCriteria(String playerName,
        ...
        StringBuilder sql = new StringBuilder(
                "SELECT p.*, t.id AS team_id, t.name AS team_name, t.continent AS team_continent " +
                        "FROM player p " +
                        "LEFT JOIN team t ON p.id_team = t.id WHERE 1=1 "
        );
